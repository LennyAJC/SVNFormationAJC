...
public class Exec {
	...
	p.setCategories(new Categories(numCategorie));
	em = getEntityManager();
	...
	p = (Produits) em.createQuery("select p from Produit p where p.idProduit='" + id + "'").getSingleResult();
	...
	Query query = em.createQuery("select p from Produits p");

	for (Object result : query.getResultList()) {
	    // p.add((Produits) result);
	    u.afficher("Nom du produit :" + ((Produits) result).getIdProduit() + " | "
		    + ((Produits) result).getNomProduit());
	}
	...
	Query query = em.createQuery("select p from Produits p  WHERE p.idProduit='" + id + "'");

	Object result = query.getSingleResult();

	System.out.println("toto");

	u.afficher("Categorie :" + ((Produits) result).getCategories());
